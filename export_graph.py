...
tf.flags.DEFINE_string('valPath', '/home/libo/DATA_REG/n2dtest/',
                       'X tfrecords file for training, default:')
tf.flags.DEFINE_string('GraphPath', '/mnt/media/libo/Brats/f_bs1.pb',
                       'X tfrecords file for training, default:')
...
tf.flags.DEFINE_integer('NUM_ID',25885,
                       'X tfrecords file for training, default:')
tf.flags.DEFINE_string('load_model','checkpoints/20211009-1006',
...
#    if XtoY:
    src = tf.placeholder(tf.float32,
        shape=[1,48, 272, 208, 1],name='src')
    tgt = tf.placeholder(tf.float32,
        shape=[1,48, 272, 208, 1],name='tgt')
    label = tf.placeholder(tf.float32,
        shape=[1,48, 272, 208, 8],name='label')
    label_t = tf.placeholder(tf.float32,
        shape=[1,48, 272, 208, 8],name='label_t')    
...
#    export_saver = tf.train.Saver()
    restore_saver = tf.train.import_meta_graph("/home/libo/regbrain/checkpoints/20211105-1616/model.ckpt-1081.meta")
  with tf.Session(graph=graph) as sess:
    ...
    output_graph_def = tf.graph_util.convert_variables_to_constants(
        sess, graph.as_graph_def(), [y.op.name,flow.op.name,l.op.name])
